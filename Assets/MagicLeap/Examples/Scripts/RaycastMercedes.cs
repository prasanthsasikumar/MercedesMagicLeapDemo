...
namespace MagicLeap
{
    ...
    /// </summary>
    public class RaycastMercedes : MonoBehaviour
    {
        ...
        private void OnButtonDown(byte controllerId, MLInputControllerButton button)
        {
            ...
            {
                //car.transform.rotation = Quaternion.Slerp(car.transform.rotation, Quaternion.Euler(car.transform.rotation.eulerAngles), Time.deltaTime * 2.5f);
                Debug.Log("Rotating");
                car.transform.Rotate(0,10, 0, Space.Self);
            }
        }

        private void OnTriggerDown(byte controllerId, float value)
        {
            car.transform.SetPositionAndRotation(_raycastController.transform.position, Quaternion.Euler(Vector3.zero));
            car.SetActive(true);
        }
